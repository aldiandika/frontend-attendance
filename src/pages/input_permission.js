...
import InputPermCard from "../components/input_perm_card";

class InputPermission extends React.Component {
  state = {
    selfAttendData: [],
    ...
    return (
      <Route
        render={() => this.state.isLoggedIn ? (
          <>
            <Helmet>
              <title>Input Izin | Aplikasi Kehadiran Divusi</title>
            </Helmet>
            ...
            <Box
              ...
              paddingRight={2}
            >
              {(this.state.permNow || this.state.attendNow) ? (
                <>
                  <Card>
                    <Box
                      sx={{
